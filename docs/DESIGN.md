# Design Document: Capability Emergence in a Tiny RL Sandbox

## 1. Purpose

This document is the method contract for **Capability Emergence in a Tiny RL Sandbox**. It specifies what is
...
One-liner: Measure whether small-model RL in a constrained deduction sandbox produces transferable capability jumps.

Hypothesis: Measure whether small-model RL in a constrained deduction sandbox produces transferable capability jumps.
...
|---|---|---|
| Config schema | `rlsand.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `rlsand.configs.loader` | Hydra compose API |
| Device | `rlsand.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `rlsand.models.registry` | Pinned revisions + hardware fit |
| Hooks | `rlsand.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `rlsand.models.generation` | Chat template path recording |
| Data | `rlsand.data` | Synthetic items, splits, manifests |
| Evaluation | `rlsand.evaluation` | Metrics, failure harness, protocols |
| Ablation | `rlsand.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `rlsand.reporting` | Aggregate → MD/TeX/figures |
| Cache | `rlsand.cache` | Versioned artifact store |
...
Mentor(s): Alana Sung
Affiliation(s): 
SPAR posting: https://github.com/alanasung/rl-capability-sandbox
...
`src/rlsand/rlsand/` are preserved across regeneration.
